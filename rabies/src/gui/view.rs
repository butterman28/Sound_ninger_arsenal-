    ...
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if let Some(_asset) = self.current_asset.read().as_ref() {
...
// ========== END: REPLACE THIS SECTION ==========
                            ...
                            for (idx, mark) in marks.iter().enumerate() {
                                ...
                                let is_currently_playing = self.is_playing.load(Ordering::Relaxed) 
                                    ...
                                    && {
                                        ...
                                        (current_pos - mark.position).abs() < 0.05
                                    };
                                ...
                                // ✅ Draw keyboard hint based on pad position
                                let key_hint = match idx {
                                    0 => "1", 1 => "2", 2 => "3", 3 => "4",
                                    4 => "Q", 5 => "W", 6 => "E", 7 => "R",
                                    8 => "A", 9 => "S", 10 => "D", 11 => "F",
                                    12 => "Z", 13 => "X", 14 => "C", 15 => "V",
                                ...
                                // Draw keyboard key hint in top-left corner
                                ...
                                if response.hovered() {
                                    egui::show_tooltip_at_pointer(ui.ctx(), egui::Id::new(format!("pad_tooltip_{}", idx)), |ui| {
                                        ...
                                        ui.label(egui::RichText::new(format!("Keyboard: {}", key_hint)).small().color(egui::Color32::YELLOW));
        ...
        // ✅ KEYBOARD PAD TRIGGERING (MPC-style layout)
        // Map keyboard keys to pad indices:
        // 1,2,3,4 → pads 1-4
        // Q,W,E,R → pads 5-8
        // A,S,D,F → pads 9-12
        // Z,X,C,V → pads 13-16
        let key_to_pad_map: Vec<(egui::Key, usize)> = vec![
            // Row 1: Number keys
            (egui::Key::Num1, 0),
            (egui::Key::Num2, 1),
            (egui::Key::Num3, 2),
        ...
        let marks = self.samples_manager.get_marks();
        for (key, pad_index) in key_to_pad_map {
            if ctx.input(|i| i.key_pressed(key)) {
                if pad_index < marks.len() {
                    let mark = &marks[pad_index];
                    if let Some(asset) = self.current_asset.read().clone() {
                        ...
                        *self.status.write() = format!("🎹 Pad #{} triggered ({:.2}s)", mark.id, time_at_mark);
                    }
                }
            }
        }
